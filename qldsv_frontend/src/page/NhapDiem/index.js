...
const NhapDiem = () => {
  ...
  const search = (evt) => {
    evt.preventDefault();
    nav(`/nhapdiem?monHocId=${monHocId}&tenSinhVien=${kw}`);
  };
  ...
  const updateDiem = (evt) => {
    ...
    const process = async () => {
      ...
      for (let field in diems) form.append(field, diems[field]);
      ...
      <div class="contend">
        <nav class="navbar navbar-1 navbar-expand-sm navbar-dark nav-menu">
          <div class="container-fluid">
            <a class="navbar-brand dark-color header-logo " href="#">
              <i class="fa-solid fa-bell icon-padding"></i>
            </a>
            <div class="collapse navbar-collapse" id="collapsibleNavbar">
              <ul class="navbar-nav">
                <li class="nav-item gv-Search-student">
                  <Form
                    onSubmit={search}
                    className="d-flex align-items-center Search-student"
                  >
                    <Form.Control
                      type="text"
                      value={kw}
                ...
                <li class="nav-item dropdown">
                  <a
                    class="nav-link dropdown-toggle dark-color"
                    href="#"
                    role="button"
                  ...
                  <ul class="dropdown-menu">
                    <li>
                      <a class="dropdown-item dark-color " href="#">
                        <i class="fa-solid fa-user icon-padding"></i>Thông Tin
                        Tài Khoản
                      </a>
                    </li>
                    <li>
                      <a class="dropdown-item dark-color" href="#">
                        <i class="fa-solid fa-key icon-padding"></i>Thay Đổi Mật
                        Khẩu
                      </a>
                    </li>
                    <li>
                      <a class="dropdown-item dark-color" href="#">
                        <i class="fa-solid fa-right-to-bracket icon-padding"></i>
                        Đăng Xuất
                      </a>
                    </li>
                  </ul>
        ...
        <div class=" point-ky">
          <div class="container mt-3 ">
                  ...
                  <table class="table" id="table-to-pdf">
                    <thead>
                      <tr>
                        <th>Mã sinh viên</th>
                        <th>Họ và tên</th>
                        {DSSinhVien[0].diemKT1 === -1 ? (
                          <></>
                        ) : (
                          <th>Điểm 1(10%)</th>
                        )}
                        {DSSinhVien[0].diemKT2 === -1 ? (
                          <></>
                        ) : (
                          <th>Điểm 2(10%)</th>
                        )}
                        {DSSinhVien[0].diemKT3 === -1 ? (
                          <></>
                        ) : (
                          <th>Điểm 3(10%)</th>
                        )}
                    ...
                    <tbody>
                      {DSSinhVien.map((sv) => {
                        let h = `/nhapdiemsinhvien?idDiem=${sv.idMonHocDangKy}&idMonHoc=${monHocId}`;
                        return (
                          <tr key={sv.idMonHocDangKy}>
                            ...
                            {sv.diemCK !== -1 ? (
                              <td>{parseFloat(sv.diemCK).toFixed(2)}</td>
                            ) : (
                              <td>-</td>
                            )}
                            {sv.diemTB !== -1 ? (
                            ...
                            {sv.monHoc.khoaMon === 1 ? (
                              <div class="btn btn-secondary mt-2">Đã khóa</div>
                            ) : (
                              <Link to={h} className="btn btn-secondary mt-2">
                                Nhập Điểm
                              </Link>
                            )}
                          </tr>
            ...
            {DSSinhVien.length !== 0 && checkKhoaDiem ? (
              <div>
                {addDiemSV ? (
                  <Spinner></Spinner>
                ) : (
                ...
                {deleDiemSV ? (
                  ...
                  <Button
                    onClick={(e) => deleteDiem(e)}
                    className=" btn-danger btn-add-delete-diem"
                    type="submit"
                  >
                    Xóa cột điểm
                  </Button>
            ...
            {DSSinhVien.length !== 0 && checkKhoaDiem ? (
              <Form onSubmit={updateDiem}>
                ...
                <div class="form-group">
                  ...
                  {checkCSV === true ? (
                    <Alert variant="danger" className="mt-3">
                      Vui lòng chọn định dạng file .CSV
                    </Alert>
                  ) : (
                    <div></div>
                  )}
